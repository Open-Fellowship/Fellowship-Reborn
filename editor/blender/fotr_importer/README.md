...
animations from the matching `.adu`. A model can also point into a sibling database -
the scarecrow's wooden pole comes from `World Common.tdu` - and those are followed
...
untextured materials without comment, which looks like a broken importer rather than a
misplaced file.
...
applies as a runtime reflection rather than through the stored UVs. They import with that
...
is visible the moment it happens rather than the next time you load the level.
...
to re-skin the Pumpkin also re-skins `pumkins_group`, and that is better to hear now than
in game.
...
armature of a rigged character counts - it finds the mesh under it.
...
rather than guess. Use **Find** to narrow the list, and set **LOD** and **Scale** to match
...
* **Moved Vertices Only** - positions and normals, nothing else. The face list, UVs, rig,
  weights and every other LOD are left exactly as they were, so this is the safe mode and
...
If the texture you point at lives in another database, the exporter says so rather than
...
faces asking for slot 19 of an array with 3 in it. That is refused rather than written,
with a message saying which mode to use instead, because the symptom in game is a crash or
garbage rather than anything that points back at the export. **One Per Material** grows the
...
of merged coplanar faces. It is not, and that is what produced the long twisted ribbons that
...
The export also warns, rather than refuses, when a mesh is much larger than the
...
* Animations carry root motion, exactly as the game authored them.
...
* Object placements carry the engine's own conventions: rotations compose in XYZ order
  ...
  facade sits a quarter turn out of true - easy to miss, because most props are trees and
  rocks with no obvious front.
