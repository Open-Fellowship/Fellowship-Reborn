...
property count suggested. **35,923 bytes in 102 functions**, of which 63 are under 200 bytes, a comfortable size to take on, and the same size class as everything matched so far. The three functions
...
why its two ordinals differ, and the reason per-function class attribution had to be
abandoned.
