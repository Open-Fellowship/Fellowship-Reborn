...
| `texture_scaling` | rfl | **on** | the mouse pointer, the circle under the health bar, the One Ring icon, the objective tick boxes, the map screen's indicator and stars and the save and load slot pictures are each drawn at their texture's own texel size, so they stay tiny at 4K. The health, ring and loading bars are the opposite: laid out correctly, then framed four and a half times too tall, so the fill sits at the top of its frame |
