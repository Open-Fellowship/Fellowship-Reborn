...
void ut_checkf(int condition, const char *format, ...)
{
    ...
    /* _vsnprintf does not terminate on truncation, so the last byte is written by hand. A
     * truncated check name is cosmetic; an unterminated one is a walk off the buffer at the
     * exact moment a failure is being reported. */
...
void ut_near(double actual, double expected, double tolerance, const char *what)
{
    ...
    /* Written as a positive test so that a NaN on either side fails instead of passing. NaN
     * compares false against everything, and a test that goes green because a number stopped
     * being a number is worse than no test. */
